...
def autocorrelation_matrix(X: torch.Tensor, lag: int):
    ...
    X_mean = torch.mean(X, dim=-1, keepdim=False).to(device).unsqueeze(-1)
    X_mean_transpose = X_mean.transpose(0, 1)
    for t in range(X.shape[1] - lag):
        # meu = torch.mean(X,1)
        x1 = torch.unsqueeze(X[:, t], 1).to(device)
        x2 = torch.t(torch.unsqueeze(torch.conj(X[:, t + lag]), 1)).to(device)
        Rx_lag += torch.matmul(x1 - X_mean, x2 - X_mean_transpose).to(device)
